...
# - 프리뷰 기본 '셀피(좌우반전)', 텍스트는 비반전
# - 캘리브 종료 시 data/ 에 .npz(데이터), .pkl(선형모델) 저장
# - 특징 벡터: 기존 12D + 추가 9D = 총 21D 저장
...
FEATURE_NAMES = [
    ...
    # +9 extra
    "roll_sin","roll_cos",       # 머리 roll
    "io_norm",                   # 양안 거리 / 얼굴 폭
    "earL","earR",               # 눈 열림 (h/w)
    "irisRL","irisRR",           # 홍채 반경 / 눈 크기
    "face_w_norm","face_h_norm"  # 얼굴 박스 크기(정규화)
]
...
def _pca_axes(pts: np.ndarray):
    ...
    ax1, ax2 = Vt[0], Vt[1]              # 단위 벡터(가로/세로 축)
    # RMS 기반 폭/높이 (안정)
...
def _eye_metrics(landmarks, eye_idxs, iris_idxs, W, H):
    """
    눈 윤곽 + 홍채로부터 다양한 지표 계산
    반환: dict(u, v, c, ax1, ax2, w, h, iris_r_norm, ear, ang)
    """
    ...
    ic = iris_pts.mean(axis=0)  # iris center
    # 정규화 오프셋 (u,v)
    u = float(np.dot(ic - c, ax1) / (w/2.0))
    v = float(np.dot(ic - c, ax2) / (h/2.0))

    # 홍채 반경(평균 L2) → 눈 크기 평균으로 정규화
    ...
    # 가로축 방향 각도 (roll proxy)
    ang = float(np.arctan2(ax1[1], ax1[0]))  # [-pi, pi]
...
def _compose_features(mL, mR, face_w, face_h, frame_W, frame_H):
    """
    mL/mR: _eye_metrics 결과
    face_w/h: 얼굴 박스 크기 (픽셀)
    """
    # --- base 12 ---
...
class Calibrator:
    ...
    def reset(self):
        ...
        self.samples_T, self.samples_IDX = [], []  # 시간/캘리브 타깃 인덱스
        self.start_t = None
    ...
    def feed(self, feat, t_now=None):
        ...
        self.samples_T.append(float(t_now if t_now is not None else time.time()))
    ...
    def predict(self, feat):
        ...
        # 차원 체크 (외부 모델 W와 현재 feat 차원 불일치 방지)
        if feat.shape[-1] != self.model.W.shape[1]:
            raise ValueError(f"Feature dim {feat.shape[-1]} != model dim {self.model.W.shape[1]}")
    ...
    def save_dataset_npz(self, out_dir=DATA_DIR, meta_extra=None):
        ...
        파일: data/gaze_samples_YYYYmmdd_HHMMSS.npz
          ...
          - Y: float32 [N, 2]
          - T: float32 [N]     (수집 시각)
          - pt_index: int32 [N] (해당 샘플의 캘리브 타깃 인덱스)
        ...
        T = np.array(self.samples_T, dtype=np.float32)
                ...
                if calib.collecting:
                    ...
                    if gaze_feat is not None and calib.has_model():
                        try:
                            ...
                            self._last = smoother.filter(np.array(pred, dtype=np.float32), t=now_t)
