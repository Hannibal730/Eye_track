    ...
    def __init__(self, shared: SharedState):
        ...
        def _probe_cameras(max_idx=10):
            ...
            for i in range(max_idx+1):
                ...
                ok = cap.isOpened()
                if ok:
                    # 간단히 한 프레임 시도 → 일부 환경에서 즉시 True여도 read 실패하는 경우 방지
                    ret, _ = cap.read()
                    if ret:
                        found.append(i)
                cap.release()
        ...
        def _fill_cam_list():
            ...
            try:
                ...
                cams = [0]
            if not cams:
                self.cmb_cam.addItem("No camera", -1)
            else:
                for idx in cams:
                    self.cmb_cam.addItem(f"Camera {idx}", idx)
            # 현재 값 반영
            with self.shared.lock:
                cur = int(getattr(self.shared, "camera_index", 0))
            # 일치 항목이 있으면 선택, 없으면 첫 항목
            set_to = 0
            for i in range(self.cmb_cam.count()):
                if int(self.cmb_cam.itemData(i)) == cur:
                    set_to = i; break
            self.cmb_cam.setCurrentIndex(set_to)
            self.cmb_cam.blockSignals(False)

        def _on_cam_changed(_):
            idx = int(self.cmb_cam.currentData())
            if idx < 0:  # No camera
                return
            with self.shared.lock:
                self.shared.camera_index = idx
                self.shared.status = f"Switching to camera {idx}"
...
class GazeWorker(threading.Thread):
    def __init__(self, shared: SharedState, args):
        ...
        # ▶ 추가: 현재 사용 중인 카메라 인덱스 추적
        self.cur_cam_index = int(args.camera)
    ...
    def run(self):
        ...
        try:
            # 초기 카메라 오픈
            cap = self._open_camera(self.cur_cam_index)
            if cap is None:
                print(f"Could not open webcam index {self.cur_cam_index}."); return
            ...
            with build_facemesh(self.args) as face_mesh:
                while not self.stop_flag.is_set():
                    with self.shared.lock:
                        ...
                        want_cam_idx   = int(self.shared.camera_index)
                        
                        # OneEuro 최신 파라미터 (UI→shared 경로가 아니라 args를 사용 중인 기존 동작 유지)
                    ...
                    # ▶ 추가: 카메라 전환 처리
                    if self.shared.pop_cmd("switch_camera") or (want_cam_idx != self.cur_cam_index):
                        new_cap = self._open_camera(want_cam_idx)
                        if new_cap is not None:
                            if cap is not None:
                                try: cap.release()
                                except Exception: pass
                            cap = new_cap
                            self.cur_cam_index = want_cam_idx
                            with self.shared.lock:
                                self.shared.status = f"Camera switched to {want_cam_idx}"
                                self.shared.substatus = ""
                        else:
                            with self.shared.lock:
                                self.shared.status = f"Failed to open camera {want_cam_idx}"
                                self.shared.substatus = "Check device index or permissions"
...
def main():
    ...
    with shared.lock:
        ...
        # ▶ 추가: 초기 웹캠 인덱스도 shared에 반영
        shared.camera_index = int(args.camera)
