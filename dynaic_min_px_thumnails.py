...
class GazeWorker(threading.Thread):
    def __init__(self, shared: SharedState, args):
        ...
        self.patch_min_w_ratio = float(args.patch_min_w_ratio)
        self.patch_min_h_ratio = float(args.patch_min_h_ratio)
    ...
    def _build_fused_feature(self, frame_bgr, uL, vL, uR, vR,
        ...
        # 스케일 기반 half-size
        base_half_w_L = max(su_pL, su_nL) * self.patch_scale_w
        base_half_h_L = max(sv_pL, sv_nL) * self.patch_scale_h
        base_half_w_R = max(su_pR, su_nR) * self.patch_scale_w
        base_half_h_R = max(sv_pR, sv_nR) * self.patch_scale_h
        # 실시간 컨투어 half-extent 대비 동적 최소 half-size(비율)
        dyn_min_half_w_L = self.patch_min_w_ratio * float(half_u_L)
        dyn_min_half_h_L = self.patch_min_h_ratio * float(half_v_L)
        dyn_min_half_w_R = self.patch_min_w_ratio * float(half_u_R)
        dyn_min_half_h_R = self.patch_min_h_ratio * float(half_v_R)
        # 최종 half-size: 스케일 기반 vs 동적 최소 중 큰 값
        half_w_L = max(base_half_w_L, dyn_min_half_w_L)
        half_h_L = max(base_half_h_L, dyn_min_half_h_L)
        half_w_R = max(base_half_w_R, dyn_min_half_w_R)
        half_h_R = max(base_half_h_R, dyn_min_half_h_R)
                    ...
                    if getattr(self.calib, "collecting", False):
                        ...
                        if finished:
                            ...
                            ds_path = self.calib.save_dataset_npz(DATA_DIR, {
                                ...
                                "patch_min_w_ratio": float(self.patch_min_w_ratio), "patch_min_h_ratio": float(self.patch_min_h_ratio),
...
def parse_args():
    ...
    p.add_argument("--patch_w", type=int, default=30, help="패치 가로(px)")
    p.add_argument("--patch_h", type=int, default=30, help="패치 세로(px)")
    ...
    p.add_argument("--patch_min_w_ratio", type=float, default=0.35,
                   help="동적 최소 half-width 비율 (û축 half-extent 대비)")
    p.add_argument("--patch_min_h_ratio", type=float, default=5.00,
                   help="동적 최소 half-height 비율 (v̂축 half-extent 대비)")
